...
get_recent_properties_flow_bp = Blueprint("get_recent_properties_flow", __name__)
logger=logging.getLogger(__name__)

api = Api(get_recent_properties_flow_bp)

processor = PayloadProcessor()
data_processor=PROCESS_WHATSAPP_DATA()
class Get_Recent_Properties_Webhook(Resource):
    def post(self):
        try:
            ...
            if body:
                ...
                if flow_token:
                    response_data=processor.process_payload_for_recent_properties(payload=decrypted_data)
...
api.add_resource(Get_Recent_Properties_Webhook, '/get_recent_properties')